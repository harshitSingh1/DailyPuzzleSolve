...
export default function ShopPage() {
  ...
  const handlePriceRangeChange = useCallback(
    (r: [number, number]) => setPriceRange(r),
    []
  );
  ...
  const filtered = useMemo(() => {
    ...
    const priceFiltered = withParsed.filter(({ parsed }) => {
      if (parsed.priceNumeric === null) return true;
      ...
      const belowMax =
        priceRange[1] >= MAX_PRICE
          ? true
          : parsed.priceNumeric <= priceRange[1];
    ...
    if (sort === "best-deal") {
      smartFiltered = smartFiltered.filter(
        ({ parsed }) => parsed.discount !== null
      );
    }

    if (sort === "featured") {
      smartFiltered = smartFiltered.filter(({ parsed }) =>
        (parsed.discount || "").toLowerCase().includes("featured")
      );
    }
    ...
    if (sort === "price-asc") {
      smartFiltered.sort(
        (a, b) =>
          (a.parsed.priceNumeric ?? Infinity) -
          (b.parsed.priceNumeric ?? Infinity)
      );
    } else if (sort === "price-desc") {
      smartFiltered.sort(
        (a, b) =>
          (b.parsed.priceNumeric ?? 0) - (a.parsed.priceNumeric ?? 0)
      );
    } else if (sort === "rating-desc") {
      smartFiltered.sort(
        (a, b) => (b.item.rating || 0) - (a.item.rating || 0)
      );
    }
    ...
    <>
      ...
      <main className="pt-6 pb-12">
        <div className="container">
          ...
          >
            <h1 className="font-display text-3xl font-extrabold sm:text-4xl mb-3">
              Puzzle Books & Brain Games
            </h1>
          ...
          <div className="mx-auto mb-8 max-w-3xl rounded-lg border border-amber-500/20 bg-amber-500/5 px-4 py-3 text-xs text-muted-foreground">
            ...
            we may earn a small commission at no extra cost to you. This helps us keep the site
            running and continue providing free daily puzzle solutions. Read our full{" "}
            <Link
              href="/disclaimer"
